...
const renderDailyProgress = () => {
    ...
    const completedCount = habitsForToday.filter(h => habitLogs[todayStr]?.[h.id]?.status === 'completed').length;
...
const updateHabitStatus = async (habitId, status) => {
    ...
    await setDoc(logRef, { userId, habitId, date: todayStr, status }, { merge: true });

    if (status === 'complete' && logForToday?.status !== 'completed') {
        await addXP(habitId);
...
const updateStreak = async (habitId, action) => {
    ...
    if (action === 'increment') {
        ...
        newStreak = (yesterdayLog?.status === 'completed') ? newStreak + 1 : 1;
...
const checkPerfectDayAchievement = async () => {
    ...
    const dayOfWeek = today.toLocaleString('en-US', { weekday: 'short' }).toLowerCase();
    const habitsForToday = habits.filter(h => h.frequency?.days?.includes('everyday') || h.frequency?.days?.includes(dayOfWeek));
    if (habitsForToday.length > 0 && habitsForToday.every(h => habitLogs[getTodayString()]?.[h.id]?.status === 'completed')) {
        await grantAchievement('perfect_day');
    }
};
...
const renderCorrelationChart = () => {
  ...
  if (!canvas) return; // Guard check BEFORE getting context
  ...
  if (sortedGrades.length === 0) {
    // You could add a message element here
    return;
  }
  
  const labels = sortedGrades.map(g => g.date);
  
  // Resolve theme colors to concrete strings
...
const renderTrendChart = (habitId) => {
  ...
  if (!canvas) return; // Guard check FIRST
  ...
  for (let i = 29; i >= 0; i--) {
    ...
    const y = log?.status === 'completed' ? 1 : (log ? 0 : null);
    points.push({ x: ds, y });
  }
...
const renderHeatmap = (habitId) => {
  ...
  for (let i = 1; i <= daysInMonth; i++) {
    ...
    if (log?.status === 'completed') {
      bgColor = cssVar('--accent');
      status = 'Completed';
    } else if (log?.status === 'skipped') {
      bgColor = cssVar('--secondary');
      status = 'Skipped';
    } else if (log) {
      bgColor = cssVar('--danger');
      status = 'Failed';
    } else {
      bgColor = cssVar('--card-border');
      status = 'No data';
    }
